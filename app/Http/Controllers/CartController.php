...
class CartController extends Controller
{
    public function index()
    {
        ...
        if(count($carts) > 0) {
            foreach($carts as $cart){
                $items[] = ['item_name' => $cart->name,'item_id' => $cart->id,'price' => $cart->options->single_price,'item_brand' => 'N/A','item_category' => 'Health','item_variant' => '','quantity' => $cart->qty,];
            }
        }
        $add_to_cart= [
            'event' => 'add_to_cart',
        ...
        \GoogleTagManager::set($add_to_cart);
    ...
    public function ajax_load_cart_data() {
                        ...
                        if(count($carts) > 0) {
                            foreach($carts as $cart) {
                                ...
                                if(!is_null($product_info)) {
                                    ...
                                    $price_info = '<span class="current__price">'
                                    .__currency().bnConv('bnComNum',optional($cart->options)->single_price).
                                    '</span>';

                                    if(optional($cart->options)->old_price > 0) {
                                        $price_info .= ' <span class="old__price">'
                                        .__currency().bnConv('bnComNum',optional($cart->options)->old_price).
                                        '</span>';
                                    }
                                    
                                    $cart_sidebar .= '<div class="minicart__product--items d-flex">
                                            <div class="minicart__thumb">
                                                <a href="'.route('single.product', [$product_info->id, Str::slug($product_info->title)]).'"><img class="shadow rounded p-1" src="'.asset('images/product/' .optional($product_info)->thumbnail_image).'" alt="prduct-img"></a>
                                            </div>
                                            <div class="minicart__text">
                                                <h3 class="minicart__subtitle h4"><a href="'.route('single.product', [$product_info->id, Str::slug($product_info->title)]).'">'
                            ...
                            $cart_sidebar .= '<div class="minicart__product--items text-center">
                                ...
                                <h3 class="py-1 px-2 mb-3"><b>'.__('messages.Your cart is empty').'</b><h3>
                                <a class="primary__btn minicart__button--link rounded-pill" href="'.route('products').'">'.__('messages.Start Shopping').'</a>
                            </div>';
                        }
                            ...
                            $cart_sidebar .= '</div>
                                                <div class="minicart__amount">
                                                    <div class="minicart__amount_list d-flex justify-content-between">
                                                        <span>'.__('messages.Sub Total').':</span>
                                                        <span><b id="side_cart_subtotal">'.__currency().bnConv('bnComNum',Cart::subtotal()).'</b></span>
                                                    </div>
                                                </div>';
        ...
        return Response()->json([
            ...
            'cart_count' => bnConv('bnNum',$cart_count),
        ]);
            ...
            $cart_sidebar .= '<div class="product product-cart">
                                <div class="product-detail">
                                    ...
                                    <div class="price-box">
                                        ...
                                        <span class="product-price">' .currency().bnConv('bnComNum',$cart->price). '</span>
                                    </div>
                                </div>
    ...
    public function update_cart(Request $request)
    {
        ...
        $output = [
            ...
            'cart_subtotal'=>bnConv('bnComNum',Cart::subtotal()),
        ];
        return Response($output);
    }
    ...
    public function checkout()
    {
        ...
        \GoogleTagManager::set($begin_checkout);
        if(count($carts) > 0) {
            $districts = District::orderBy('id', 'ASC')->get();
            return view('user.pages.checkout-3', compact('carts', 'districts')); 
        }
        else {
